...
namespace PotterKata.Tests
{
    public class Tests
    {
        ...
        [TestCase(new int[] { 0, 0, 0, 1 }, 31.2, TestName = "3 identical + 1 non identical")]
        [TestCase(new int[] { 1, 0, 0, 0 }, 31.2, TestName = "3 identical + 1 non identical (other order)")]
        [TestCase(new int[] { 0, 0, 0, 0, 1 }, 39.2, TestName = "4 identical + 1 non identical")]
        public void TwoDistinctWithSeveralIdenticalBooks(int[] books, double expected)
        {
            Assert.AreEqual(expected, new Basket().GetPrice(books));
        }

        [TestCase(new int[] { 0, 0, 1, 1, 2 }, 36.8, TestName = "3 distinct incl. 2 books with repetition")]
        [TestCase(new int[] { 0, 0, 1, 1, 2, 3 }, 40.8, TestName = "4 distinct incl. 2 books with repetition")]
